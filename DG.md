...
### 1.2 必要資訊準備
- **SSH 公鑰**: 用於安全存取實例（可選，留空則停用 SSH 登入）
- **API 金鑰**: 16+ 字元的安全金鑰（如: `my-secure-api-key-2024`）
- **Compartment**: 目標部署區間
- **區域**: 偏好的 OCI 區域

> **SSH 公鑰選填說明：**
> - 有填寫：可用 `ssh opc@YOUR_INSTANCE_IP` 連線 VM 進行維護、檢查日誌、手動操作。
> - 未填寫：SSH 登入停用，僅能透過 OCI 控制台與 API 管理服務，若需除錯請重新部署並啟用 SSH。
